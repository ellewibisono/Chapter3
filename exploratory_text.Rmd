...
## Take a peek into the data (per 24 March 2020)

### Comparing prices between vessels and WPPs 
In this data dump, we have three expenses (fuel, ice, and bait). Does certain vessels need to spend more to go fishing? 


```{r cost, include=FALSE}
...
```{r plot costs-point, echo=FALSE}
ggplot()+ geom_point(data=cost, aes(x=fishing_gear, y=expenses, color=cost_type), alpha=0.7)+
 scale_x_discrete(labels=c('DL','GN','LL','MG','TR'))+
  facet_wrap(~wpp1) 
...
###Legend 
DL: dropline; GN: Gillnet; LL: Longline; MG: Mix-gears; TR: Trap 
###Results: Costs are almost all the same for each fishing gear between WPPs, with notable exceptions: 
* in WPP 718: 
+ longline: much higher fuel and ice costs 
+ gillnet: much higher fuel cost
* in WPP 715: 
+ dropline: higher fuel and bait costs 


```{r plot costs-boxplot, echo=FALSE}
ggplot()+ geom_boxplot(data=cost, aes(x=fishing_gear, y=expenses))+
  scale_x_discrete(labels=c('DL','GN','LL','MG','TR'))+
